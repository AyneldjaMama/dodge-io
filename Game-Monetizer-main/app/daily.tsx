...
export default function DailyScreen() {
  ...
  const { stats, recordDailyGame, recordNearMiss, recordShieldUsed, recordAdWatched, getDailySeed } = useGame();
  ...
  const html = getGameHTML({ dailySeed: seed });

  const submitScore = useCallback(async (score: number, time: number) => {
    try {
  ...
  const handleMessage = useCallback(
    (event: { nativeEvent: { data: string } }) => {
      try {
        ...
        if (msg.type === "playerDied") {
          ...
          if (continueUsedRef.current || !adAvailable) {
    ...
    [recordDailyGame, recordNearMiss, recordShieldUsed, adAvailable, showDeadScreen]
  );
    ...
    <View style={styles.container}>
        ...
        >
          ...
          {adAvailable && (
            <Pressable
              ...
              style={({ pressed }) => [styles.watchAdBtn, pressed && { opacity: 0.85, transform: [{ scale: 0.97 }] }]}
              testID="watch-ad-btn"
            >
      ...
      {gameState === "dead" && (
        ...
        >
          ...
          {stats.dailyHighScore > 0 && finalScore >= stats.dailyHighScore && (
